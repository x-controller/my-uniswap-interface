import { arrayify } from '@ethersproject/bytes' // 排列
import { parseBytes32String } from '@ethersproject/strings' // 解析字节32字符串
import { Currency, Token } from '@uniswap/sdk-core'
import { CHAIN_INFO, L2_CHAIN_IDS, SupportedChainId, SupportedL2ChainId } from 'constants/chains'
import { useMemo } from 'react' // 备忘录

import { createTokenFilterFunction } from '../components/SearchModal/filtering' // 创建令牌过滤器函数
import { nativeOnChain } from '../constants/tokens' // 链的主网币
import { useAllLists, useCombinedActiveList, useInactiveListUrls } from '../state/lists/hooks'
// 所有列表、组合活动列表、非活动列表URL
import { WrappedTokenInfo } from '../state/lists/wrappedTokenInfo' // 包装令牌信息
import { NEVER_RELOAD, useSingleCallResult } from '../state/multicall/hooks' // 多重调用 / 钩子
// 永不重新加载，单次调用结果
import { useUserAddedTokens } from '../state/user/hooks' // 用户添加的令牌
import { isAddress } from '../utils' // 是否是地址
// 令牌地址映射，不支持的令牌列表
...
// 将令牌映射减少为标准地址 <-> 令牌映射，可选地包括用户添加的令牌
      ...
      return (
        userAddedTokens
          ...
          .reduce<{ [address: string]: Token }>(
            ...
            // 必须创建一个副本，因为reduce会修改映射，我们不希望在每次迭代中都创建一个副本
            { ...mapWithoutUrls }
          )
      )
    }
...
export function useAllTokens(): { [address: string]: Token } {
  const allTokens = useCombinedActiveList() // 使用组合代币列表
